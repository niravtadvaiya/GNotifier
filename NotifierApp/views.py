	...
	if request.method == 'POST':
	  try:
		...
		if trim(request.POST['NexmoFrom']) is None or trim(request.POST['NexmoFrom']) == '':
			Error = True
			messages.error(request,"please select Nexmo From Number!")
...
def ajax_validator(request):
	...
	if not api_key and  not api_secret:
		django_html = {'error':'true','html':str('Please enter a valid Nexmo API Key and Secret!')}
		return HttpResponse(json.dumps(django_html))
	if not api_key:
		django_html = {'error':'true','html':str('Please enter a valid Nexmo API Key!')}
		return HttpResponse(json.dumps(django_html))
	if not api_secret:
		django_html = {'error':'true','html':str('Please enter a valid Nexmo Secret!')}
		return HttpResponse(json.dumps(django_html))
	try:
		...
		django_html = {'error':'true','html':"Invalid API and Secret!"}
		return HttpResponse(json.dumps(django_html))
	except Exception as err:
